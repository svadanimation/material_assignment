...
def apply_shaders(shader_filepath, assignment_filepath, vray_proxy, name=''):
    ...
    if (not os.path.isfile(shader_filepath) 
        ...
        mc.warning(f'Could not find shader or assignment file \n \
                    shader {shader_filepath} \n \
                    assignment {assignment_filepath}')
        return False
    ...
    abc_filename = filename = mc.getAttr(f"{vray_proxy}.fileName")
    if not abc_filename or not os.path.isfile(abc_filename):
        print (f'{abc_filename}  on {vray_proxy} is not a valid abc path')
        return False
